 ...
 */
#include <linux/band.h> //itay itay itay
#include <linux/mm.h>
#include <linux/nmi.h>
...
pick_next_task:
	...
	if(!first_iteration){
        ...
        band = findBand(next->pid);
        
        if (band != NULL)
        {
          
            if (band->curr_pids[2] != next->pid && band->curr_pids[2] != -1)
            {
               
                task_bass = find_task_by_pid(band->curr_pids[2]);
                
                if(task_bass != NULL){
                   
                    if (task_bass->state == TASK_RUNNING)
                    {
