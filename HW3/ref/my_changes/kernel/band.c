...
struct Band *findBand(pid_t member)
{
    ...
    struct Band *band = NULL; // Initialize band to NULL
    
    

    if (first_iteration == 1 || list_empty(&g_list_band)) {
    ...
    return NULL; // Return NULL if no matching band was found in the entire list.
}
